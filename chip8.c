        ...
        if      ((instr & 0xF000) == 0x0000){ // misc
            ...
            for(int cur_y = y; cur_y < y + s; cur_y++){
                ...
                for(int cur_x = x; cur_x < x + 8; cur_x++){
                    ...
                    if(!c8->regs[15] && c8->display[cur_y%HEIGHT][cur_x%WIDTH] && bit){
                        c8->regs[15] = 1;
                    }
            ...
            switch(instr & 0x00FF){
                ...
                case(0x29): // point I to the sprite for hexadecimal character in vr
                    c8->index_register = c8->regs[(instr & 0x0F00) >> 8] * 5; // 5 is because each char is 5 rows tall.
                    break;
